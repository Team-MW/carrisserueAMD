...
const navigationMenuTriggerStyle = cva(
  "flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none text-white focus:bg-white focus:text-black disabled:opacity-50 disabled:pointer-events-none bg-transparent hover:bg-white hover:text-black data-[state=open]:bg-white data-[active]:bg-white h-10 py-2 px-4 group w-max",
  );
...
>(({ className, ...props }, ref) => (
  <div className={cn("absolute -translate-x-1/2 -left-20 top-full flex justify-center")}>
    <Viewport
      className={cn(
        "origin-top-left data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:zoom-in-90 data-[state=closed]:zoom-out-95 relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-md border border-slate-200 bg-white shadow-lg md:w-[var(--radix-navigation-menu-viewport-width)] ",
        className,
      )}
      ref={ref}
      {...props}
